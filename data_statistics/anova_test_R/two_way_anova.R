...
#2 MT-ID双因素方差分析
sub_data2<-read.csv("MT_line_chart_data_all.csv")
demo2 <- sub_data2[sub_data2$state=="amputee",]
demo2

#2.1 正态性检验
with(demo2,tapply(MT, model, shapiro.test))   #Shapiro-Wilk检验

#2.2 方差齐性检验    
leveneTest(MT~model,demo2)   #两组数据可用bartlett.test,3组或3组以上用leveneTest

#2.3 双因素方差分析aov
AOV2 <- aov(MT ~ filter*model,demo2)
summary(AOV2)

#2.4 双因素方差分析lm,anova
AOV2=lm(MT ~ filter + model + filter:model,data=demo2)
anova(AOV2)
summary(AOV2)  

#2.5 双因素方差分析lmer,anova 
AOV2 <- lmer(MT ~ model +  filter +(1|ID), data=demo2)
anova(AOV2)
summary(AOV2)
