...
demo1 <- sub_data1[sub_data1$state=="healthy" & sub_data1$metrics=="success_rate",]    #throughput/success_rate/overshoot
demo1



############################################################方法1：
summary(aov(var_value ~ filter*model + Error(number/(filter*model)), data=demo1))
...
Lme.mod <- lme(var_value ~ filtermodel, random=~1|number,
               correlation=corCompSymm(form=~1|number),
               data=demo1)
