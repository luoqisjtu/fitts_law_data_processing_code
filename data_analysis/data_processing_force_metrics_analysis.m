...
%         IDs =[5.17;4.37;3.59;5.95;4.17;4.95];   %IDs =[5.17;4.37;3.59;4.95;4.17;5.95];   352614  当在实验设置中改变不同ID的task顺序时，此处也要相应调整顺序！！！！！！   下次实验改为IDs =[4.37;5.17;3.59;5.95;4.17;4.95];
        ...
        for k=1:level
            ...
            Data_force = abs(force_val(1:(length(force_val)-500),2)); %有效压力数据—去掉最后的5s的数据，也就是500个数    %根据unity中设置的延时时间调整时，此处也要相应调整顺序！！！！！！   
 ...
 %%           
            ...
            if(completion_time_re(k)>14.8||collision_p(k)~=0||movement_time(k)<=0)            %%t>14.8或碰到禁止区域或没有从起始位置出发对应的trial记为fail  
...
%            completion_time_nonfail(:,k) = completion_time_ct(:,k)(completion_time_ct(:,k)~=0);  %a(a==0) = []; a(find(a==0)) = [];  b = a(a ~= 0);  b = a(find(a ~= 0));
 %% 计算压力曲线平滑度（光滑度smoothness）           
            force_diff1 = diff(Data_force); %求一阶导(一阶差分)
...
%             force_diffaver_re(k) = mean(force_diffaver(:,k));
       ...
       %===================================================================================           
            ...
            if(force_rms_diff1(q,k)>2)  %————————去掉两个异常值(model1)
...
%% 求压力曲线相对于reference曲线的均方根误差Root Mean Squared Error   
...
%             RMSE_force_hand(q,k) = sqrt(sum((force_mm-force_target(k,:)*force_sigmoid').^2)/numel_force);   
            ...
            if(RMSE_force_hand(q,k)>90)  %————————去掉异常值
...
%% 求压力曲线force curve与reference curve的相似性similarity      
        ...
        %================================================================求两组向量的线性相关性
%             Rcor_force_mat = corrcoef(force_mm,force_target(k,:)*force_sigmoid'); %注意行列向量转换   
            Rcor_force_mat = corrcoef(force_mm,force_ref_resa_hand);    %求两组向量的线性相关性
         ...
         %================================================================求夹角余弦-度量两组向量之间夹角的大小
            Sim_force(q,k) = dot(force_mm,force_ref_resa_hand)/(norm(force_mm)*norm(force_ref_resa_hand)); %求夹角余弦-度量两组向量之间夹角的大小   sim(x,y)=cos(x,y)
